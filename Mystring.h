...
{
public:
	MyString();//생성자					// 비어 있는 MyString() 생성
	MyString(const char* init);//문자열 생성자		// 맨 뒤에 널 캐릭터'\0'가 들어 있는 문자열로부터 초기화
	MyString(const MyString& str);//복사 생성자	// MyString의 다른 instance로부터 초기화
	~MyString();//소멸자
