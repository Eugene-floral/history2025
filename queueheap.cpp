...
struct Patient
{
	...
	int time;	  // <- 도착시간, 정렬할 때의 2차적인 키(key)로 사용됨, 중증도가 같을 때는 먼저 온 순서 오케이.
	const char* name;
	...
	friend bool operator >= (const Patient& a, const Patient& b)
	{
		if (a.severity == b.severity) // 우선순위가 같을 때는 시간을 기준으로 다시 판단 (선착순)
			return a.time <= b.time; // 숫자로써 time이 작은 것이 먼저 왔다는 의미니까 최종 우선순위가 더 높다. -> a가 time이 더 작으면,true
		else
			return a.severity > b.severity; // 우선순위가 큰 쪽 기준 a가 더 크면 
	}
	...
	{
		cout<< "name: "<< p.name <<", 중증도: "<< p.severity << ", time: " << p.time<< endl;
		return os;
	}
};
